  ...
  const handleFilterChange = (key: string, value: string | boolean) => {
    ...
    const filtered = authors.filter((author) => {
      return (
        ...
        (newFilters.language === '' || author.language === newFilters.language) &&
        (newFilters.selfPublished === null || author.selfPublished === newFilters.selfPublished)
      );
    });
  ...
  return (
    <section className="max-w-7xl mx-auto px-8 py-5">
      ...
      <div className="mb-8 flex flex-wrap gap-4">
        ...
        <select
          className="p-2 border rounded"
          onChange={(e) => handleFilterChange('selfPublished', e.target.value === 'true')}
          value={filters.selfPublished === null ? '' : filters.selfPublished.toString()}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAuthors.map((author) => (
          <div key={author.id} className="border p-4 rounded shadow">
            ...
            <p className="mb-1">Publishing: {author.selfPublished ? 'Self-Published' : 'Traditional'}</p>
